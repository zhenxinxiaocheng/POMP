...
### 功能介绍
SunoAPI 模块集成了 Suno AI 音乐生成服务，允许用户通过 API 生成原创音乐。

### 环境要求
- 需要有效的 Suno API Key
- 可以通过以下两种方式之一提供：
  - 环境变量：设置`SUNO_API_KEY`环境变量
  - 配置文件：在`util/config.json`文件中的`SunoAPI.apiKey`字段中配置
...
- **参数**:
  ...
  - `apiKey`: Suno API 密钥（可选，如果已设置环境变量）
  - `prompt`: 音乐生成提示词（必填）
  - `title`: 音乐标题（可选）
  - `tags`: 音乐标签（可选）
  - `options`: 其他可选参数（JSON 格式）

- **示例请求**:
  ```
  GET /sunoapi?action=generate&apiKey=your_api_key&prompt=一首欢快的流行歌曲&title=快乐时光
  ```
...
- **示例请求**:
  ```
  GET /sunoapi?action=get&apiKey=your_api_key&taskId=your_task_id
  ```
